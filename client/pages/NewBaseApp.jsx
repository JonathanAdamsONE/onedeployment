...
const spriteSize = 24; //Variable controlling the size of the sensor sprites
 ...
 *
 * @memberof Autodesk.DataVisualization.UI.NewBaseApp
 */
 ...
 * @component
 * @param {Object} props
 * @param {Object} props.appData Data passed to the NewBaseApp.
 ...
 * @alias Autodesk.DataVisualization.UI.NewBaseApp
 */

import { MyDataAdapter } from "./MyDataAdapter.js";
import { forEach, max } from "lodash";

export default function NewBaseApp(props) {
    /**
    ...
    useEffect(() => {
        document.title = "ONE Creative Pilot Project";
     ...
     * @alias Autodesk.DataVisualization.UI.NewBaseApp#initializeDataStore
     * @private
     */
    ...
    async function onModelLoaded(viewer, data) {
        const [session, masterDataView] = await initializeDataStore();  //session.datastore.devicemodels.deviceid.devices
    ...
    function printdbIds(suffixes, model) {
        let modelIndex = 0;

        suffixes.forEach((suffix) => {
            modelIndex = 0;

            model.myData.instanceTree.nodeAccess.nameSuffixes.forEach((modelSuffix) => {
                if (suffix == modelSuffix) {
                    for (let i = 0; i < 113407; i++) {
                        if (model.myData.instanceTree.nodeAccess.dbIdToIndex[i] == modelIndex) {
                            console.log(i);
                        }
                    }
                }
    ...
    function getModelCentre(dbId, fragList)
    {
        const fragBounds = new THREE.Box3();
        const bounds = new THREE.Box3();
        let fragId = {};

        for (let i = 0; i < fragList.fragments.fragId2dbId.length; i++) {
            if (fragList.fragments.fragId2dbId[i] == dbId) {
                fragId = i;
            }
        }
...
2377060
        ...
        function setupRoomSensors() {

            
            session.dataStore.deviceModels.forEach((deviceModel) => {

                //If the sensor list is set up to include general room sensors
                if (deviceModel.id == "GeneralRoomSensor") {

                    let i = 1;
                    let floorCounter = 0;

                    //find the fragId of each room in the model
                    shadingData._children.forEach((floor) => {
                        floor._children.forEach((room) => {
                            room.fragIds.forEach((fragId) => {

                            //position data for the new sensor (should be the centre of the associated rooms)
                                let neutralPosition = new THREE.Box3();
                                
                                neutralPosition.x = 0;
                                neutralPosition.y = 0;
                                neutralPosition.z = 0;

                            //Throw all of the new data into a device model ready to be added to the general sensor list
                                let newDevice = {
                                    deviceModel: deviceModel,
                                    sensorTypes: deviceModel.propertyIds,
                                    _dbId: room.dbIds[0],
                                    _floorId: floor.id,
                                    _lastActivityTime: null,
                                    _name: room.name + " General Sensor",
                                    _position: neutralPosition,


                                    name: room.name + " General Sensor",
                                    dbId: room.dbIds[0],
                                ...
                                appState.session._dataStore._deviceModels.GeneralRoomSensor._devices[newDevice._name] = newDevice;
                            });
                        });
                    });
            ...
            session._dataStore._deviceModels.GeneralRoomSensor = appState.session._dataStore._deviceModels.GeneralRoomSensor; 
        ...
        session.dataStore.deviceModels.forEach((deviceModel) => {
            ...
            deviceModel.devices.forEach((device) => {
                if (device.dbId >= 0) {
                    ...
                    newShadingNode.addPoint(newShadingPoint);

                    let i = 0;
                    let addedToRoom = false;
                    let objectName = {};
                    shadingData.children.forEach((floor) => {
                        if (floor.id == device.floorId  || floor.id == device._floorId) {
                            floor.children.forEach((object) => {
                                object.dbIds.forEach((dbId) => {
                                    if (dbId == device.dbId)
                                    {
                                        objectName = object.name;
                                        object.addPoint(newShadingPoint);
                                        addedToRoom = true;
                                    }
                                });
                            });
                            ...
                            if (!addedToRoom) {
                                shadingData.children[i].addChild(newShadingNode);
                            }
         ...
         * @alias Autodesk.DataVisualization.UI.NewBaseApp#generateViewables
         * @private
         */
         ...
         session.dataStore.deviceModels.forEach((deviceModel) => {
            ...
            currentViewables.push(modelViewable);
        });
            ...
            if (event.hovering && currAppState.dbId2DeviceIdMap) {
                ...
                if (device) {
                    ...
                    setHoveredDeviceInfo({
                        id: deviceId,
                        xcoord: mappedPosition.x + 70,  //70 is the width of the left bar in pixels
                        ycoord:
                            mappedPosition.y +
                            vertificalOffset -
         ...
         * initialized from the {@link NewBaseApp#initializeDataStore}
         * @private
         */
     ...
     * @alias Autodesk.DataVisualization.UI.NewBaseApp.#handleTimeRangeUpdated
     */
     ...
     * @alias Autodesk.DataVisualization.UI.NewBaseApp.#handleCurrTimeUpdated
     */
    ...
    function setEnergyConsumption(deviceModel, startTime, endTime) {
        ...
        deviceModel.devices.forEach((device) => {
            ...
            if(dataOverTime[device.id]) {
                ...
                let dataTotal = 0;
                let index = 0;
                dataOverTime[device.id].properties.Power.seriesData.forEach((dataEntry) => {
     ...
     * @alias Autodesk.DataVisualization.UI.NewBaseApp#getSensorValue
     * @private
     */
    ...
    //const deviceList = session._dataStore.deviceModels;
    ...
    function getRefreshViewableStyle(styleId, styleMap) {
        return styleMap[styleId] || styleMap["default"];
    }

    function isViewable(deviceId, deviceModel) {
    ...
    async function refreshViewables(shadingData, viewableModels) {
        let dbId = 1;
        const dbId2DeviceIdMap = {};
        const deviceId2DbIdMap = {};
        ...
        for (let i = 0; i < leafNodes.length; i++) {
            ...
            for (let j = 0; leaf.shadingPoints && j < leaf.shadingPoints.length; j++) {
                ...
                viewableModels.forEach((viewableModel) => {

                    if (isViewable(device.id, viewableModel)) {
                        const style = getRefreshViewableStyle(device.contextData.styleId, styleMap);
                        const position = {
                            x: device.position.x,
                            y: device.position.y,
                            z: device.position.z
        ...
        appState.dataVizExtn.showHideViewables(renderSettings.showViewables, renderSettings.occlusion);

        if (!renderSettings.showTextures) {
            appState.dataVizExtn.hideTextures();
        }

        const propertyMap = appState.session.dataStore.getPropertiesFromDataStore();
        const defaultHeatmapOptions = Object.assign({}, heatmapOptions, {
            selectedPropertyId: Array.from(propertyMap.keys())[0] || "",
        });
        setHeatmapOptions(defaultHeatmapOptions);
        ...
        if (props.surfaceShadingConfig && props.surfaceShadingConfig.gradientSetting) {
            let gradientSettings = props.surfaceShadingConfig.gradientSetting;
            for (let deviceType in gradientSettings) {
                appState.dataVizExtn.registerSurfaceShadingColors(
                    deviceType,
                    gradientSettings[deviceType],
                    0.7
                );
        ...
        let state = Object.assign({}, appState, {
            ...
            dataVizExtn: appState.dataVizExtn,
            session: appState.session,
            masterDataView: appState.masterDataView,
            deviceId2DbIdMap: deviceId2DbIdMap,
            dbId2DeviceIdMap: dbId2DeviceIdMap,
            propertyMap: propertyMap,
            shadingData: shadingData,
    ...
    let redrawSprites = false;
    currentViewables.forEach((deviceModel) => {
        if (deviceModel.viewable != deviceModel.nextState) {
            redrawSprites = true;
        }
    });
    ...
    if (redrawSprites) {
        ...
        currentViewables.forEach((refreshDevice) => {
            if (refreshDevice.nextState) {
                appState.session.dataStore.deviceModels.forEach((deviceModel) => {
                    if (deviceModel.id == refreshDevice.id) {
                        
                        viewableModels.push(deviceModel);

                    }
                });
            }

            refreshDevice.viewable = refreshDevice.nextState;
        });

        refreshViewables(appState.shadingData, viewableModels);
        redrawSprites = false;
    }
    ...
    if (devicesToQuery.length > 0 && !devicesToQuery[devicesToQuery.length-1]) {
        devicesToQuery.length = devicesToQuery.length-1;
    }
    ...
    function handleQueryCompleted(eventArgs) {
        // The following hook causes NewBaseApp to repaint every time a device
        ...
        <React.Fragment>
            ...
            <div className = "ONElogo">
                <a href="https://www.oneltd.com/">    
                    <img src={oneLogoSvg} alt="One Logo"/>
                </a>
            </div>
